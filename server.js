...
const emailConfirmationHandler = require('./api/email-confirmation');
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'GET') {
    ...
    if (url === '/email-confirmation' || url === '/email-confirmation.html' || url.startsWith('/email-confirmation/')) { serveFile(res, 'email-confirmation.html'); return; }
  }
  ...
  if (url === '/api/email-confirmation') { await emailConfirmationHandler(req, res); return; }
